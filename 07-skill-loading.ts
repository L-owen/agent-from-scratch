/**
 * s07_skill_loading — 在 s06 基础上新增 skill 按需加载
 *
 * s06 的工具集和子 Agent 不变，s07 新增：
 *   + SKILLS_DIR / SKILL_REGISTRY — 扫描 skills/ 目录，注册技能元数据
 *   + _parseFrontmatter() — 解析 SKILL.md 的 YAML frontmatter
 *   + _scanSkills() — 启动时扫描 skills/ 子目录，填充注册表
 *   + listSkills() / buildSystem() — 动态构建 SYSTEM prompt（含技能目录）
 *   + loadSkill() — 运行时按需加载完整技能内容（通过 tool_result 注入）
 *   + load_skill 工具 — Agent 可调用加载指定技能
 *
 * 两级加载设计：
 *   Layer 1（便宜，常驻）：SYSTEM prompt 包含技能名+描述（~100 tokens/skill）
 *   Layer 2（按需，调用时）：load_skill 返回完整 SKILL.md 内容（~2000 tokens/skill）
 *
 * Usage:
 *   cp .env.example .env  # fill in DASHSCOPE_API_KEY and MODEL_ID
 *   npx tsx 07-skill-loading.ts
 */
...
async function main() {
  ...
  const ask = (): Promise<string> =>
    new Promise((resolve) => rl.question("\x1b[36ms07 >> \x1b[0m", resolve));
