/**
 * s06_subagent — 在 s05 基础上新增 task 工具（子 Agent）
 *
 * s05 的工具集不变，s06 新增：
 *   + task 工具 — spawn 一个子 Agent，独立 messages[]，只回传摘要
 *   + spawnSubagent() — 子 Agent 拥有独立循环，30 轮安全限制
 *   + SUB_TOOLS / SUB_HANDLERS — 子 Agent 有基础工具，但没有 task（禁止递归）
 *   + extractText() — 从最后一条消息中提取文本结论
 *
 * 上下文隔离：子 Agent 的中间过程不污染主 Agent 的 messages[]
 * 安全策略不跳过：子 Agent 工具调用也经过 PreToolUse hook
 *
 * Usage:
 *   cp .env.example .env  # fill in DASHSCOPE_API_KEY and MODEL_ID
 *   npx tsx 06-subagent.ts
 */
...
const SYSTEM = `You are a coding agent at ${WORKDIR}. For complex sub-problems, use the task tool to spawn a subagent. Before starting any multi-step task, use todo_write to plan your steps.`;
...
async function main() {
  ...
  const ask = (): Promise<string> =>
    new Promise((resolve) => rl.question("\x1b[36ms06 >> \x1b[0m", resolve));
